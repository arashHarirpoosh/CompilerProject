...
_lr_productions = [
  ...
  ('declist -> dec','declist',1,'p_declist','Parser.py',18),
  ('declist -> declist dec','declist',2,'p_declist','Parser.py',19),
  ('declist -> <empty>','declist',0,'p_declist','Parser.py',20),
  ('dec -> vardec','dec',1,'p_dec','Parser.py',30),
  ('dec -> funcdec','dec',1,'p_dec','Parser.py',31),
  ('type -> INTEGER','type',1,'p_type','Parser.py',40),
  ('type -> FLOAT','type',1,'p_type','Parser.py',41),
  ('type -> BOOLEAN','type',1,'p_type','Parser.py',42),
  ('iddec -> ID','iddec',1,'p_iddec','Parser.py',52),
  ('iddec -> ID LSB exp RSB','iddec',4,'p_iddec','Parser.py',53),
  ('iddec -> ID ASSIGN exp','iddec',3,'p_iddec','Parser.py',54),
  ('idlist -> iddec','idlist',1,'p_idlist','Parser.py',64),
  ('idlist -> idlist COMMA iddec','idlist',3,'p_idlist','Parser.py',65),
  ('vardec -> type idlist SEMICOLON','vardec',3,'p_vardec','Parser.py',74),
  ('funcdec -> type ID LRB paramdecs RRB block','funcdec',6,'p_funcdec','Parser.py',83),
  ('funcdec -> VOID ID LRB paramdecs RRB block','funcdec',6,'p_funcdec','Parser.py',84),
  ('paramdecs -> paramdecslist','paramdecs',1,'p_paramdecs','Parser.py',93),
  ('paramdecs -> <empty>','paramdecs',0,'p_paramdecs','Parser.py',94),
  ('paramdecslist -> paramdec','paramdecslist',1,'p_paramdecslist','Parser.py',103),
  ('paramdecslist -> paramdecslist COMMA paramdec','paramdecslist',3,'p_paramdecslist','Parser.py',104),
  ('paramdec -> type ID','paramdec',2,'p_paramdec','Parser.py',113),
  ('paramdec -> type ID LSB RSB','paramdec',4,'p_paramdec','Parser.py',114),
  ('varlist -> vardec','varlist',1,'p_varlist','Parser.py',123),
  ('varlist -> varlist vardec','varlist',2,'p_varlist','Parser.py',124),
  ('varlist -> <empty>','varlist',0,'p_varlist','Parser.py',125),
  ('block -> LCB varlist stmtlist RCB','block',4,'p_block','Parser.py',135),
  ('stmtlist -> stmt','stmtlist',1,'p_stmtlist','Parser.py',143),
  ('stmtlist -> stmtlist stmt','stmtlist',2,'p_stmtlist','Parser.py',144),
  ('stmtlist -> <empty>','stmtlist',0,'p_stmtlist','Parser.py',145),
  ('lvalue -> ID','lvalue',1,'p_lvalue','Parser.py',152),
  ('lvalue -> ID LSB exp RSB','lvalue',4,'p_lvalue','Parser.py',153),
  ('stmt -> RETURN exp SEMICOLON','stmt',3,'p_stmt','Parser.py',162),
  ('stmt -> exp SEMICOLON','stmt',2,'p_stmt','Parser.py',163),
  ('stmt -> block','stmt',1,'p_stmt','Parser.py',164),
  ('stmt -> WHILE LRB exp RRB stmt','stmt',5,'p_stmt','Parser.py',165),
  ('stmt -> FOR LRB exp SEMICOLON exp SEMICOLON exp RRB stmt','stmt',9,'p_stmt','Parser.py',166),
  ('stmt -> IF LRB exp RRB stmt elseiflist','stmt',6,'p_stmt','Parser.py',167),
  ('stmt -> IF LRB exp RRB stmt elseiflist ELSE stmt','stmt',8,'p_stmt','Parser.py',168),
  ('stmt -> PRINT LRB ID RRB SEMICOLON','stmt',5,'p_stmt','Parser.py',169),
  ('elseiflist -> ELIF LRB exp RRB stmt','elseiflist',5,'p_elseiflist','Parser.py',184),
  ('elseiflist -> elseiflist ELIF LRB exp RRB stmt','elseiflist',6,'p_elseiflist','Parser.py',185),
  ('elseiflist -> <empty>','elseiflist',0,'p_elseiflist','Parser.py',186),
  ('exp -> lvalue ASSIGN exp','exp',3,'p_exp','Parser.py',196),
  ('exp -> exp operator exp','exp',3,'p_exp','Parser.py',197),
  ('exp -> exp relop exp','exp',3,'p_exp','Parser.py',198),
  ('exp -> const','exp',1,'p_exp','Parser.py',199),
  ('exp -> lvalue','exp',1,'p_exp','Parser.py',200),
  ('exp -> ID LRB explist RRB','exp',4,'p_exp','Parser.py',201),
  ('exp -> LRB exp RRB','exp',3,'p_exp','Parser.py',202),
  ('exp -> ID LRB RRB','exp',3,'p_exp','Parser.py',203),
  ('exp -> SUB exp','exp',2,'p_exp','Parser.py',204),
  ('exp -> NOT exp','exp',2,'p_exp','Parser.py',205),
  ('operator -> OR','operator',1,'p_operator','Parser.py',222),
  ('operator -> AND','operator',1,'p_operator','Parser.py',223),
  ('operator -> SUM','operator',1,'p_operator','Parser.py',224),
  ('operator -> SUB','operator',1,'p_operator','Parser.py',225),
  ('operator -> MUL','operator',1,'p_operator','Parser.py',226),
  ('operator -> DIV','operator',1,'p_operator','Parser.py',227),
  ('operator -> MOD','operator',1,'p_operator','Parser.py',228),
  ('const -> INTEGERNUMBER','const',1,'p_const','Parser.py',242),
  ('const -> FLOATNUMBER','const',1,'p_const','Parser.py',243),
  ('const -> TRUE','const',1,'p_const','Parser.py',244),
  ('const -> FALSE','const',1,'p_const','Parser.py',245),
  ('relop -> GT','relop',1,'p_relop','Parser.py',256),
  ('relop -> LT','relop',1,'p_relop','Parser.py',257),
  ('relop -> NE','relop',1,'p_relop','Parser.py',258),
  ('relop -> EQ','relop',1,'p_relop','Parser.py',259),
  ('relop -> GE','relop',1,'p_relop','Parser.py',260),
  ('relop -> LE','relop',1,'p_relop','Parser.py',261),
  ('explist -> exp','explist',1,'p_explist','Parser.py',274),
  ('explist -> explist COMMA exp','explist',3,'p_explist','Parser.py',275),
]
